...
type
  ...
  Event* = object
    ...
    of evCreateContainer:
      spec*: Container
    of evRunCheck, evTest, evCleanLEBackups, evFlushStdout, evUpdateProcMetrics:
      discard
...
proc registerHandler*(manager: EventManager, ek: EventKind,
...
proc triggerRepeat*(manager: EventManager, e: Event,
    sleepSeconds: Natural) {.async.} =
  proc f() {.async.} =
