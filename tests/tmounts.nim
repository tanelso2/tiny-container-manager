import
    tiny_container_manager/yaml_utils
...
proc ofYaml(n: YNode, t: typedesc[Mount]): Mount =
    expectYMap:
        ...
        of mkTmpfs:
            ...
            raise newException(ValueError, "unknown kind")
...
assert m2.mountPoint == m.mountPoint
assert m2.kind == m.kind

let c3: Con = ofYaml(cy, Con)
assert c3.name == c2.name
assert len(c3.mounts) == 1
assert c3.mounts[0].name == m2.name
